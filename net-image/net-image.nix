...
in {
  ...
  options.netImage = {
    rootDirectoryName = mkOption {
      default = "${config.netImage.directoryBaseName}-${config.system.nixos.label}-${pkgs.stdenv.hostPlatform.system}";
      description = ''
    ...
    nfsRoot = mkOption {
      type = types.str;
      default = "192.168.1.87:/mnt/nfsshare/${config.netImage.directoryBaseName}-${config.system.nixos.label}-${pkgs.stdenv.hostPlatform.system}";
      description = ''
