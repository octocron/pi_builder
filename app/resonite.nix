...
{pkgs, ...}: let
  ...
  rmlLaunchArg =
    if (resoniteModLoader == true)
    then "-LoadAssembly Libraries/ResoniteModLoader.dll"
    else "";

  installHeadlessTweaks =
    if (resoniteModLoader == true)
    then
      if (headlessTweaks == true)
      then ''
  ...
  UpdateMods =
    if (resoniteModLoader == true)
    then
      pkgs.writeShellScriptBin "UpdateMods" ''
